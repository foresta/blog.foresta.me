...
これは、i32 型が Copy Trait を実装しているためです。Copy Trait を実装された型の値が代入されるときにはコピーする動作になります。所有権を写す方は `ムーブせマンティクス` とよばれ、値を複製する方は`コピーせマンティクス` とよばれます。

なので、以下のように Foo 方に Copy Trait を実装すると正常に実行することができます。
...
`println!` マクロが x を借用しようとしてエラーになっています。これは、以下のような借用規則があるためです。
...
ライフタイムの短い値を参照するということは、初期化されていない値を参照してしまうおそれがあるということなので、これを防ぐのが 1 番目の借用規則になります。
